...
cargo_manifest := "native/langelic_archive/Cargo.toml"

# Interactive release: pick patch/minor/major, roll the CHANGELOG, tag & push.
release:
    elixir scripts/release.exs

# Run the full test suite (builds the NIF from source).
test:
...
check:
    ...
    LANGELIC_ARCHIVE_BUILD=true mix compile --warnings-as-errors
    mix credo --strict
